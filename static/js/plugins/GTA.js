...
function generateRandomNodes(numNodes) {
  ...
  for (let i = 1; i <= numNodes; i++) {
    nodes.push({ id: i, label: `${i}` }); //label: `N ${i}`
  }
  return nodes;
}
...
addNodeButton.addEventListener('click', () => {
    ...
    const newNodeLabel = `${newNodeId}`; // `N ${newNodeId}`;
...
smoothCheckbox.addEventListener("change", () => {
  options.edges.smooth = smoothCheckbox.checked;
  network.setOptions(options);
});

const smoothLabel = document.createElement("label");
smoothLabel.textContent = " Smooth edges ";
...
physicsLabel.style.marginRight = "10px";
smoothLabel.style.marginRight = "10px";
...
toolbox.appendChild(smoothLabel);
      ...
      for (let j = 0; j < row.length; j++) {
        ...
        if (!isNaN(value) && value === 1 && i < j) {
          ...
          if (!newNodesData.some((node) => node.id === from)) {
            newNodesData.push({ id: from, label: `${from}` }); //label: `N ${from}`
          }
          if (!newNodesData.some((node) => node.id === to)) {
            newNodesData.push({ id: to, label: `${to}` }); //label: `N ${to}`
          }
        }
      }
    }
...
adjacencyMatrixInputLabel.style.marginBottom = '30px';
